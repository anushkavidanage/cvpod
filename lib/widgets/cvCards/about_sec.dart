...
Container buildAboutSec(Map aboutData) {
  return Container(
    ...
    child: Column(
      ...
      children: [
        for (AboutItem aboutRec in aboutData.values) ...[
          Text(aboutRec.name.toUpperCase(),
              style: const TextStyle(fontSize: 35, color: backgroundWhite)),
          Text(aboutRec.position,
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            aboutDataElement(aboutRec.address, 12),
          ]),
          const SizedBox(height: 5.0),
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Icon(Icons.email, size: 18, color: backgroundWhite),
            const SizedBox(width: 10.0),
            aboutDataElement(aboutRec.email, 12),
            const SizedBox(width: 20.0),
            const Icon(Icons.phone, size: 18, color: backgroundWhite),
            const SizedBox(width: 10.0),
            aboutDataElement(aboutRec.phone, 12),
            const SizedBox(width: 20.0),
            // const Icon(FontAwesomeIcons.linkedin,
            const Icon(Icons.facebook, size: 18, color: backgroundWhite),
            const SizedBox(width: 10.0),
            aboutDataElement(aboutRec.linkedin, 12),
          ]),
        ]
      ],
    ),
